...
class FixValidationMessages extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:fix-validation-messages';
     ...
     * @var string
     */
    protected $description = 'Memperbaiki pesan validasi dengan menambahkan terjemahan bahasa Indonesia';
    ...
    public function handle()
    {
        ...
        // Pastikan direktori lang/id ada
        if (!File::exists(lang_path('id'))) {
            File::makeDirectory(lang_path('id'), 0755, true);
        }

        // Buat file validation.php jika belum ada
        if (!File::exists(lang_path('id/validation.php'))) {
            $this->createValidationFile();
        }

        // Buat file id.json jika belum ada
        if (!File::exists(lang_path('id.json'))) {
            $this->createJsonFile();
        }

        // Perbarui konfigurasi app.php
        $this->updateAppConfig();

        $this->info('Pesan validasi berhasil diperbaiki!');
    }

    /**
     * Membuat file validation.php
     */
    private function createValidationFile()
    {
        $content = <<<'PHP'
<?php
...
PHP;
        ...
        File::put(lang_path('id/validation.php'), $content);
        $this->info('File validation.php berhasil dibuat!');
    }

    /**
     * Membuat file id.json
     */
        ...
        $content = <<<'JSON'
{
    ...
    "validation.min.string": "Kolom ini minimal harus :min karakter.",
    "validation.max.string": "Kolom ini tidak boleh lebih dari :max karakter.",
    "validation.numeric": "Kolom ini harus berupa angka.",
    "validation.date": "Kolom ini bukan tanggal yang valid.",
    "validation.image": "File harus berupa gambar.",
    "validation.mimes": "File harus bertipe: :values.",
    "validation.max.file": "Ukuran file tidak boleh lebih dari :max kilobytes."
}
JSON;

        File::put(lang_path('id.json'), $content);
        $this->info('File id.json berhasil dibuat!');
    }

    /**
     * Memperbarui konfigurasi app.php
     */
    private function updateAppConfig()
    {
        ...
        // Pastikan locale sudah diset ke 'id'
        if (strpos($config, "'locale' => env('APP_LOCALE', 'id')") === false) {
            $config = str_replace(
        ...
        // Pastikan fallback_locale sudah diset ke 'id'
        if (strpos($config, "'fallback_locale' => env('APP_FALLBACK_LOCALE', 'id')") === false) {
            $config = str_replace(
        ...
        // Pastikan faker_locale sudah diset ke 'id_ID'
        if (strpos($config, "'faker_locale' => env('APP_FAKER_LOCALE', 'id_ID')") === false) {
            $config = str_replace(
        ...
        $this->info('Konfigurasi app.php berhasil diperbarui!');
    }
}
